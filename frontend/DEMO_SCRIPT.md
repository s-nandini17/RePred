...
**Project**: RepPred — Protein Benchmark  
...
### 0:00 – 0:25 | Introduction & Scientific Thesis
...
* **Narration**:
  > *"Welcome to RepPred — an interactive scientific benchmark exploring how protein representations impact stability prediction. Predicting mutation-driven stability change ($\Delta\Delta G$) depends heavily on how a model 'sees' a protein. RepPred evaluates five fundamental representation paradigms using the exact same 3,433 experimental FireProt mutations under strict protein-held-out cross-validation."*

---

### 0:25 – 0:50 | The Five Representation Paradigms
* **Visual**: Home Page — Five Representation Progression Cards & Explorer
* **Action**: Explore the 5 representation paradigms: Sequence (252D), ESM-2 (1280D), WT 3D (131D), Contact Map (107D), and Protein GNN.
* **Narration**:
...
### 0:50 – 1:35 | Mutation Explorer & 3D Molecular Centerpiece
...
* **Narration**:
  > *"Let's explore a real FireProt mutation: M1A in Protein G (PDB 1PGA). Notice the 3D molecular viewer displaying the authentic experimental wild-type PDB structure. The mutation site at residue 1 is highlighted in glowing chartreuse. Following scientific rigor, we never generate or display artificial mutant coordinates. On the right, we see the experimental ground truth $\Delta\Delta G = +1.98\text{ kcal/mol}$ (destabilizing)."*

---

### 1:35 – 2:00 | Representation Switching & Model Predictions
...
* **Narration**:
  > *"As we switch representations, the explanatory panel dynamically reveals how each representation perceives the mutation — from local mass density (10Å sphere) and C$\alpha$ B-factors to 8Å contact neighbor counts. Below, we see saved model predictions across all paradigms alongside exact experimental ground truth."*

---

### 2:00 – 2:20 | Benchmark Leaderboards & Nuanced Scientific Findings
* **Visual**: Benchmark Leaderboard Page (`/benchmark`)
* **Action**: Highlight the **Official Held-Out Test Leaderboard** ($N=350$) and the **Grouped Cross-Protein Leaderboard** ($N=3,083$).
* **Narration**:
...
### 2:20 – 2:35 | Interpretability & Data Provenance
...
1. Open `http://127.0.0.1:5173/` in Google Chrome.
2. Click **`Presentation Mode`** in the top navigation bar to toggle dark theme or clean layout for presentations.
