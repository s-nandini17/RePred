...
import { Dna, Cpu, Box, Share2, Sparkles, Info, CheckCircle2, AlertTriangle, Layers, Eye } from 'lucide-react';
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', color: '#11110F' }}>
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <h1 style={{ fontSize: '2.4rem', fontWeight: 900, letterSpacing: '-0.03em', margin: '0 0 8px 0', color: '#11110F' }}>
          Explore a Mutation
        </h1>
        <p style={{ fontSize: '1.05rem', color: '#52504a', margin: 0 }}>
          Follow an authentic FireProt mutation across five distinct protein representation paradigms.
        </p>
      </div>
      ...
      >
        ...
        <div style={{ flex: '1 1 280px' }}>
          <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 800, color: '#11110F', textTransform: 'uppercase', marginBottom: '6px' }}>
            1. Select Protein / Experimental PDB
          </label>
          <select
          ...
          >
            {availableMutations.map((m, idx) => (
              <option key={`${m.experiment_id}_${m.mutation}_${idx}`} value={m.experiment_id}>
                {m.mutation} (PDB Res #{m.pdb_position}) &bull; &Delta;&Delta;G = {m.ddG.toFixed(2)} kcal/mol [{m.split.toUpperCase()}]
              </option>
            ))}
          </select>
        </div>
      ...
      {currentMutation && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(440px, 1fr))', gap: '32px' }}>
          ...
          <div>
            ...
            {/* Explicit WT Structure Rule Banner */}
            <div
              style={{
                marginTop: '16px',
                background: '#F8F7F3',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                <span style={{ background: '#11110F', color: '#D8FF4F', fontSize: '0.7rem', fontWeight: 800, padding: '2px 6px', borderRadius: '2px', fontFamily: 'monospace' }}>
                  EXPERIMENTAL WT STRUCTURE
                </span>
                <strong style={{ color: '#11110F' }}>Mutation site: {currentMutation.wt_aa}{currentMutation.pdb_position} &rarr; {currentMutation.mut_aa}</strong>
              </div>
              <div>
                The 3D viewer displays <strong>authentic experimental wild-type PDB coordinates</strong> ({currentMutation.pdb_id}) and highlights the residue where FireProt reports the mutation ({currentMutation.wt_aa}{currentMutation.pdb_position}{currentMutation.mut_aa}).
              </div>
              <div style={{ marginTop: '6px', fontStyle: 'italic', color: '#11110F', fontWeight: 600 }}>
                The mutation is information attached to the WT structure; it is NOT a mutant structure. Zero AlphaFold, zero predicted mutant coordinates.
              </div>
            </div>
          </div>

          {/* RIGHT COLUMN: MUTATION DETAILS, STORY EXPLANATION, REPRESENTATION PANELS & PREDICTION */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {/* MUTATION DETAILS CARD */}
            <div style={{ background: '#11110F', border: '1px solid #11110F', borderRadius: '6px', padding: '20px', color: '#F8F7F2', boxShadow: '2px 2px 0px #11110F' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#D8FF4F', fontFamily: 'monospace' }}>
                    {currentMutation.wt_aa} &rarr; {currentMutation.mut_aa}
                  </span>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', textAlign: 'center', background: '#1c1b18', padding: '12px', borderRadius: '4px' }}>
                <div>
                ...
                <div>
                  <div style={{ fontSize: '0.7rem', color: '#a3a199', textTransform: 'uppercase' }}>Experimental WT PDB</div>
                  <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#D8FF4F' }}>{currentMutation.pdb_id} ({currentMutation.chain})</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.7rem', color: '#a3a199', textTransform: 'uppercase' }}>Experimental &Delta;&Delta;G</div>
                  <div style={{ fontSize: '0.95rem', fontWeight: 900, color: currentMutation.ddG < 0 ? '#10b981' : '#f43f5e', fontFamily: 'monospace' }}>
            ...
            {/* "HOW DOES THIS REPRESENTATION SEE THIS MUTATION?" DYNAMIC STORYTELLING */}
            <div style={{ background: '#F8F6F0', border: '1px solid #11110F', borderRadius: '6px', padding: '16px 20px', boxShadow: '2px 2px 0px #11110F' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                <Eye size={16} color="#11110F" />
                <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#11110F', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  How this representation sees the mutation:
                </span>
              </div>
              <div style={{ fontSize: '1.05rem', fontWeight: 800, color: '#11110F' }}>
                {activeRepTab === 'sequence' && '"Looks at sequence context and mutation chemistry."'}
                {activeRepTab === 'esm' && '"Encodes the protein sequence into a learned 1280-dimensional representation."'}
                {activeRepTab === '3d' && '"Looks at the physical environment around the mutation."'}
                {activeRepTab === 'contact_map' && '"Looks at which residues are spatially connected to the mutation."'}
                {activeRepTab === 'gnn' && '"Propagates information through the experimental WT protein graph."'}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '4px' }}>
                <button
                  ...
                  style={{ padding: '8px 12px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 700, background: activeRepTab === 'sequence' ? '#11110F' : '#ECEAE4', color: activeRepTab === 'sequence' ? '#D8FF4F' : '#5a5852', border: '1px solid #11110F', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}
                >
                  <Dna size={14} /> Sequence (252D)
                </button>
                <button
                  onClick={() => setActiveRepTab('esm')}
                  style={{ padding: '8px 12px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 700, background: activeRepTab === 'esm' ? '#11110F' : '#ECEAE4', color: activeRepTab === 'esm' ? '#D8FF4F' : '#5a5852', border: '1px solid #11110F', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}
                >
                  <Cpu size={14} /> ESM-2 (1280D)
                </button>
                <button
                  onClick={() => setActiveRepTab('3d')}
                  style={{ padding: '8px 12px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 700, background: activeRepTab === '3d' ? '#11110F' : '#ECEAE4', color: activeRepTab === '3d' ? '#D8FF4F' : '#5a5852', border: '1px solid #11110F', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}
                >
                  <Box size={14} /> WT 3D (131D)
                </button>
                <button
                  onClick={() => setActiveRepTab('contact_map')}
                  style={{ padding: '8px 12px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 700, background: activeRepTab === 'contact_map' ? '#11110F' : '#ECEAE4', color: activeRepTab === 'contact_map' ? '#D8FF4F' : '#5a5852', border: '1px solid #11110F', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}
                >
                  <Share2 size={14} /> Contact Map (107D)
                </button>
                <button
                  onClick={() => setActiveRepTab('gnn')}
                  style={{ padding: '8px 12px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 700, background: activeRepTab === 'gnn' ? '#11110F' : '#ECEAE4', color: activeRepTab === 'gnn' ? '#D8FF4F' : '#5a5852', border: '1px solid #11110F', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}
                >
            ...
            {/* PREDICTION PANEL */}
            <div style={{ background: '#F8F7F3', border: '1px solid #11110F', borderRadius: '6px', padding: '20px', boxShadow: '2px 2px 0px #11110F' }}>
              ...
              {currentPredictions ? (
                <div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '10px', marginBottom: '14px' }}>
                    <div style={{ background: '#ffffff', border: '1px solid #11110F', padding: '10px', borderRadius: '4px', textAlign: 'center' }}>
                      <div style={{ fontSize: '0.7rem', color: '#5a5852', fontWeight: 700 }}>GNN Pred</div>
                      <div style={{ fontSize: '0.95rem', fontWeight: 900, color: '#9333ea', fontFamily: 'monospace' }}>
                    ...
                    <div style={{ background: '#ffffff', border: '1px solid #11110F', padding: '10px', borderRadius: '4px', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: '0.95rem', fontWeight: 900, color: '#e11d48', fontFamily: 'monospace' }}>
                    ...
                    <div style={{ background: '#ffffff', border: '1px solid #11110F', padding: '10px', borderRadius: '4px', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: '0.95rem', fontWeight: 900, color: '#4f46e5', fontFamily: 'monospace' }}>
                        {currentPredictions.sequence !== undefined ? `${currentPredictions.sequence.toFixed(2)} kcal/mol` : 'N/A'}
                      </div>
                    </div>
                  </div>

                  <div style={{ background: '#ECEAE4', border: '1px solid #11110F', padding: '10px 14px', borderRadius: '4px', fontSize: '0.75rem', color: '#5a5852', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
